...
public class DiscountServiceImpl implements DiscountService {
    ...
    public Page<DiscountDto> getByCriteria(DiscountSearchCriteria searchCriteria) {
        ...
        List<Long> discountId = new ArrayList<>();
        ...
        result.forEach(x -> discountId.add(x.getId()));
        List<RateSetter> rateList = reviewDAO.getRateByDiscountId(discountId);
    ...
    private List<DiscountDto> setRate(List<RateSetter> rtList, List<DiscountDto> dtoList) {
        for (DiscountDto d : dtoList) {
            for (RateSetter r : rtList) {
                if (d.getId() == r.getDiscountId()) {
                    d.setRate((double) r.getRate());
                    break;
                }
            }
        }
