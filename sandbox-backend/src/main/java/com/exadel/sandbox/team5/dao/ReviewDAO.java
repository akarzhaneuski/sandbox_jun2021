...
public interface ReviewDAO extends JpaRepository<Review, Long> {
    ...
    @Query(value = """
            SELECT new com.exadel.sandbox.team5.util.PairLongDouble(r.discount.id, AVG(r.rate))
            FROM Review r
            WHERE r.discount.id IN (:list)
            GROUP BY r.discount.id""")
    List<PairLongDouble> getRateByDiscountId(@Param("list") Set<Long> discountId);
}
